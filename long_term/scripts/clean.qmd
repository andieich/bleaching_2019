...
dat %>% 
  ...
  filter(year == 2009, marine_area == "Pihaena", habitat == "Barrier reef", transect == 1, season == "Oct")
```

Typo. 2009-08-08 should be 2009-09-08 (consecutive row_numbers)
...
cheakced in table, 17404 shoudl be t1 not 3
...
Make wide and long again to get data for all genera. Otherwise, for some surveys, no corals remain \> the survey would be excluded when sub-setting to corals (tested, will happen). Like that, it's 0%
