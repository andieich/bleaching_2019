...
Calculate the relative change to better compare between genera

```{r}
pred_gen_19_rel <- predict_before_after_rel(.data = dat_genus_delta %>% 
                       filter(year %in% 2019:2020) %>% 
                       droplevels(), 
                     .model = m_ccover_genus_19, 
                     .get_summary = F,
...
pred_gen_20_rel <- predict_before_after_rel(.data = dat_genus_delta %>% 
                       filter(year %in% 2020:2021) %>% 
                       droplevels(), 
                     .model = m_ccover_genus_20, 
                     .get_summary = F,
...
pred_gen_21_rel <- predict_before_after_rel(.data = dat_genus_delta %>% 
                       filter(year %in% 2021:2022) %>% 
                       droplevels(), 
                     .model = m_ccover_genus_21, 
                     .get_summary = F,
...
pred_gen_22_rel <- predict_before_after_rel(.data = dat_genus_delta %>% 
                       filter(year %in% 2022:2023) %>% 
                       droplevels(), 
                     .model = m_ccover_genus_22, 
                     .get_summary = F,
...
pred_gen_rel <- bind_rows(pred_gen_19_rel,
                  pred_gen_20_rel,
                  pred_gen_21_rel,
                  pred_gen_22_rel) %>% 
    mutate(coast = factor(coast, levels = c("West", "North", "East"))) %>% 
  group_by(coast) %>% 
  mutate(marine_area = fct_reorder(.f = marine_area, .x = rel_change, .fun = mean, .desc = T)) %>% 
  ungroup()
```

```{r}
plot_change_site_gen_rel <- pred_gen_rel %>%
    ggplot(aes(x = rel_change*100, y = marine_area, col = genus))+
    stat_pointinterval(position = position_dodge(.5), )+
    geom_vline(xintercept = 0, linetype = "11")+
    facet_wrap(~coast, ncol = 1, scales = "free_y")+
    labs( x = expression(Delta~"Cover (%"~yr^-1*")"), y = NULL)+
...
plot_change_site_gen_rel
```

Absolute change per year per coastal orientation

```{r}
plot_change_coast_gen <- pred_gen %>%
    ggplot(aes(x = ate*100, y = coast, col = genus))+
...
plot_change_coast_gen


ggsave(filename = "plot_change_coast_gen.pdf",
       plot = plot_change_coast_gen, 
       path = here("long_term/plots/"),
       width = 20, height = 12, units  = "cm")
```

Relative change per coast

```{r}
plot_change_coast_gen_rel <- pred_gen_rel %>%
    ggplot(aes(x = rel_change*100, y = coast, col = genus))+
  geom_vline(xintercept = 0)+
  geom_vline(xintercept = c(-50, 50), linetype = "11")+
    stat_pointinterval(position = position_dodge())+
    
    labs(x = expression(Delta~"Cover (%"~yr^-1*")"), y = NULL)+
    scale_colour_manual(values = cols_genera, name = NULL)+
    theme_minimal()+
  facet_grid(coast~comparison, scales = "free_y")+
    theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(colour = "black",  fill='transparent'),
        plot.title = element_text(hjust = 0.5, face="bold"),
