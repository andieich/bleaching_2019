...
make_url <- function(lat1,
  ...
  } else if (parameter == "MUR_SST"){
    base_url <- "https://coastwatch.pfeg.noaa.gov/erddap/griddap/jplMURSST41.nc?analysed_sst"
  } 
...
download_all <- function(lat1,
  ...
  if (!parameter%in% c("CRW_SST", "CRW_DHW", "MUR_SST")){
    
    stop(paste("Parameter has to be CRW_SST, CRW_DHW, or  MUR_SST but is", parameter))
...
find_closest_maxdhw <- function(data, lat, lon, col = "max_dhw") {
