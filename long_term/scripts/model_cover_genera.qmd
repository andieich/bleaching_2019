...
Make new df and check range of proportion (Beta not defined for 1 and 0)
...
0 occurs Either transform or use ordered beta regression
...
```{r}
summary(m_ccover_genus_19)#4 d trans, rhat, ESS good
summary(m_ccover_genus_20)#10 d trans, rhat, ESS good
summary(m_ccover_genus_21)#37 d trans, rhat good , ESS ok
summary(m_ccover_genus_22)#38 d trans, rhat good, ESS ok
```
...
```{r}
summary(m_ccover_genus_19_dhw)
pp_check(m_ccover_genus_19_dhw, ndraws = 100)
bayes_R2(m_ccover_genus_19_dhw)
```
