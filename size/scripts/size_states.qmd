...
#predict data with model along sequence of diameter
dat_pred <- model %>% 
  epred_draws(newdata = expand_grid(
    dia = seq(0, 30, length = 200),
    site = unique(data$site)))


#summarise predictions
dat_predS <- dat_pred %>% 
  group_by(dia,site, .category) %>% 
...
plot <- dat_predS %>%
  mutate(.category = recode(.category,
                            ...
                            "shrunk" = "partial mortality")) %>% 
  ggplot(aes(x = dia))+
  geom_ribbon(aes(x = dia, ymin = l95, ymax = u95, fill = .category), alpha = .2)+
  geom_ribbon(aes(x = dia, ymin = l90, ymax = u90, fill = .category), alpha = .2)+
  geom_ribbon(aes(x = dia, ymin = l80, ymax = u80, fill = .category), alpha = .2)+
  ...
  xlim(0,30)+
  labs(y = "Probability", x = "Colony diameter (cm)",
       title = title)+
  theme_minimal()+
  theme_andi()+
...
```{r}
m_state_16_18 <- brm(state ~  s(dia, by = site, k = 4), 
                     data = dat_state %>% 
...
```{r}
m_state_18_20 <- brm(state ~  s(dia, by = site, k = 4), 
                     data = dat_state %>% 
...
```{r}
m_state_20_22 <- brm(state ~  s(dia, by = site, k = 4), 
                     data = dat_state %>% 
