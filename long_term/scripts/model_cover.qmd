...
post_pred <- fitted(m_ccover,
  ...
  # # remove the draw index column
  ungroup() %>%
  select(marine_area, coast, delta_2020, delta_2021, delta_2022, delta_2023) %>% 
...
pl <- post_predS %>%
  ...
  geom_point(aes(y = median), position = position_dodge(width = .1))+
    labs( x = expression(Delta~"Cover (%"~yr^-1*")"), y = "Cover (%) compared to before")+
    theme_andi()+
    theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
plot_slope<- slope %>%
  ...
  labs(x = expression(Delta~"% cover"~DHW^-1), y = "Density")+
