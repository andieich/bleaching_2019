...
```{r}
m_ccover_genus<- brm(counts|trials(50) ~ marine_area*factor(year)*genus,
                     ...
                     seed = 123,
                     file = here("long_term/models/m_ccover_genus_binom"),
                     backend = "cmdstanr")
```
...
post_pred <- epred_draws(m_ccover_genus,
                    newdata = new_data) %>%
  ungroup() %>% 
  select(.draw, marine_area, genus, coast, year, .epred) %>% 
...
```{r}
m_ccover_genus_19_dhw <- brm(counts|trials(50) ~ 0 +  max_dhw_29 * genus * factor(year)+ 
                         ...
                         threads = threading(2, static = T),
                         seed = 123,
                         file = here("long_term/models/m_ccover_genus_19_dhw_binom"),
                         backend = "cmdstanr")
```
