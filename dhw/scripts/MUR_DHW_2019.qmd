...
```{r}
map_FrenchPolynesia <- st_read(
  dsn = here("dhw/data/maps/polynesia_map/PYF_adm0.shp")
) %>%
  st_as_sf()
```
...
map_moorea <- ggplot() +
  ...
  coord_sf(
    xlim = c(moorea_box$lon_min, moorea_box$lon_max),
...
dat_sites <- dat_sites %>%
  rowwise() %>%
  mutate(
    max_dhw = find_closest_maxdhw(
      data = dat_moorea_mur_sst_dhwS,
      lat = lat,
      lon = lon,
      "max_dhw"
    )
...
write.csv(
  dat_sites,
  file = here("dhw/data/dat_sites_maxdhw_mur.csv"),
  row.names = F
)
```
...
map_corals <- st_read(
  dsn = here(
    "dhw/data/maps/Coralreef_distribution_polynesia/Coralreef_distribution_polynesia.shp"
  ),
  quiet = T
) %>%
  st_as_sf()
...
plot_dhw_map <- ggplot() +
  ...
  geom_text_repel(
    data = sites,
    aes(x = lon, y = lat, label = site, hjust = hjust),
    col = "white",
  ...
  geom_text(
    aes(x = -149.845, y = -17.538, label = "Mo'orea"),
    fontface = "italic",
    size = 4,
  ...
  coord_sf(xlim = c(lon_min, lon_max), ylim = c(lat_min, lat_max), expand = F) +
...
Fot graphical abstract

```{r}
plot_dhw_map_GA <- ggplot() +
  geom_raster(
    data = dat_moorea_mur_sst_dhwS %>%
      filter(lat >= lat_min, lat <= lat_max, lon >= lon_min, lon <= lon_max),
    aes(x = lon, y = lat, fill = max_dhw)
