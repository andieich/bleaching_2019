...
```{r}
predraw_states_before <- plot_model(
  data = dat_state %>%
    filter(comparison == "2016_2018"),
  model = m_state_16_18,
  title = "2016 - 2018"
...
predraw_states_during <- plot_model(
  data = dat_state %>%
    filter(comparison == "2018_2020"),
  model = m_state_18_20,
  title = "2018 - 2020"
...
predraw_states <- bind_rows(predraw_states_before, predraw_states_during) %>%
...
plot_sub <- predraw_states %>%
  mutate(.category = recode(.category, "died" = "complete mortality")) %>%
  ggplot(aes(x = log_area)) +
  geom_ribbon(
    aes(x = log_area, ymin = l95, ymax = u95, fill = .category),
    alpha = .2
...
```{r}
ggsave(
  filename = "plot_SI_exp_mortality_bleaching.pdf",
  plot = plot_sub,
  path = here("size/plots"),
  width = 30,
  height = 10,
