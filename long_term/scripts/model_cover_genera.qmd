...
dat_c %>%
  ...
  group_by(genus,year) %>% 
  summarise(points_coral = sum(points_coral),
...
dat_genus_delta <- dat_genus_delta %>%
  mutate(
    marine_area = case_when(
      marine_area == "Entre 2 baies" ~ "Entre 2 baies",
...
pred_marine_area %>%
  filter(year != 2019) %>%
  group_by(genus, year) %>%
  summarise(cover = median(percent),
            l_95 = quantile(percent, 0.025, na.rm = T),
            u_95 = quantile(percent, 0.975, na.rm = T)) %>%
    mutate(
    m_SI = paste0(
...
col_coast = c("West" = "#0A9F9DFF", "North" = "#CEB175FF", "East" = "#E54E21FF")
...
dat_above0 <- slope_gen %>%
  group_by(genus) %>%
  summarize(above_0 = above_0(slope_val), below_0 = 100*round(1 -above_0,4)) %>%
