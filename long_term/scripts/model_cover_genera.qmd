...
m_ccover_genus <- brm(
  counts |
    trials(50) ~ max_dhw * factor(year) * genus
  t,
...
```{r}
summary(m_ccover_genus)#Rhats ok
pp_check(m_ccover_genus, ndraws = 100) # good

library(performance)

r2_bayes(m_ccover_genus)
# Conditional R2: 0.889 (95% CI [0.875, 0.901]) # both
# Marginal R2: 0.484 (95% CI [0.387, 0.661]) # only main
...
new_data_ahs_max <- dat_genus_delta %>% 
  dplyr::select(genus, year) %>% 
  distinct() %>% 
  expand_grid(max_dhw = seq(min(dat_genus_delta$max_dhw),max(dat_genus_delta$max_dhw), length = 100))

pred_ahs_max <- epred_draws(m_ccover_genus,
                    newdata = new_data_ahs_max, re_formula = NA) %>%
  ungroup() %>% 
  dplyr::select(.draw, max_dhw, genus, year, .epred) %>% 
...
pred_ahs_maxS <- pred_ahs_max %>% 
 group_by(year, max_dhw, genus) %>% 
  ...
  mutate(gr = paste(factor(max_dhw), name, sep = "_")) %>% 
  ungroup()
...
pred_ahs_max_labels <- pred_ahs_maxS %>% 
  filter(year == 2023) %>% 
  group_by(name, genus) %>% 
...
plot_change_ahs_max <- pred_ahs_maxS %>%
  ggplot() +
  geom_line(aes(
    x = year,
    y = value,
  ...
  geom_text(data = pred_ahs_max_labels,
            aes(x = 2023.15, y = mean, label = name),
            size = 3) +
  scale_colour_viridis(name = expression(AHS[max])) +
  facet_grid( ~ genus) +
...
new_data_marine_area <- dat_genus_delta %>% 
  dplyr::select(genus, year, marine_area) %>% 
  distinct() %>% 
...
pred_marine_area <- epred_draws(m_ccover_genus,
  ...
  ungroup() %>% 
  dplyr::select(.draw, marine_area, genus, year, .epred, max_dhw) %>% 
  pivot_wider(names_from = year, values_from = .epred) %>% 
  clean_names() %>% 
  mutate(perc_2019 = 100*(x2019/50),
         perc_2020 = 100*(x2020/50),
...
plot_change_marine_area <- pred_marine_areaS %>%
  ...
  facet_grid(genus ~ gr_n, scales = "free") +
  theme_andi() +
...
ggsave(filename = "plot_change_cover_gen.pdf",
       ...
       path = here("long_term/plots/"),
       width = 15*0.8, height = 25*0.8, units  = "cm")
```
